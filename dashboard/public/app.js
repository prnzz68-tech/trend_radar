...
function render() {
  ...
  const telegramStatus=!data.telegramConfigured?'Telegram-бот ещё не подключён к сайту. Администратору нужно добавить токен в закрытые настройки хостинга и подключить webhook.':`Telegram-бот доступен всем: /start включает ежедневную подборку, /stop отключает её, /digest и /global показывают тренды. Подписчиков: ${data.telegramSubscriberCount || 0}.${data.telegramLastSentDate?` Последняя отправка: ${data.telegramLastSentDate}.`:''}`;
