...
function render() {
  ...
  $('#connection').textContent=data.hasYouTubeKey?'YouTube API подключён. Автоматическая проверка выполняется раз в 24 часа, пока локальный сервер работает.':'YouTube API пока не подключён. Для автоматического сбора задайте YOUTUBE_API_KEY перед запуском сервера.';
