...
## Локальная панель по коротким видео

В папке [`dashboard/`](dashboard/) находится локальная панель для маркетингового анализа онлайн-обучения. Она показывает три видео дня, публичные просмотры и вовлечённость, ссылки на источники и гипотезу для рекламного теста. YouTube подключается по ключу API; TikTok, Instagram и Threads пока пополняются вручную. Запуск и ограничения описаны в [dashboard/README.md](dashboard/README.md).

Панель запускается отдельно от описанного ниже Python-сервиса. Сейчас у них нет общей базы данных или общей авторизации. Локальные ключи и данные панели исключены из Git.
